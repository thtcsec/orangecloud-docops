import type { DocumentType, RelationshipType } from "@shared/domain";
...
import { getCase, linkCaseDocument } from "../db/repositories/cases";
...
export function relationshipFromDocumentType(
...
export async function uploadDocument(
  ...
  // Single buffer: arrayBuffer once → hash + R2 (avoid stream re-copy).
  ...
  if (input.caseId) {
    const caseRow = await getCase(
      env.DOCOPS_DB,
      principal.organizationId,
      input.caseId,
    );
    if (!caseRow) {
      throw Object.assign(new Error("Case not found"), { code: "CASE_NOT_FOUND" });
    }
  }
  ...
  try {
    ...
    if (input.caseId) {
      const relationshipType = relationshipFromDocumentType(documentType);
      await linkCaseDocument(env.DOCOPS_DB, {
    ...
    try {
      await env.DOCUMENTS_BUCKET.delete(r2ObjectKey);
    } catch {
      // best-effort cleanup
    }
    try {
